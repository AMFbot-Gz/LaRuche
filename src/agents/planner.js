 ...
 *
 * v3 : Skills chargés dynamiquement via skillLoader (4 niveaux de priorité)
 *      getRelevantSkills(intent) → prompt ciblé, moins de tokens, meilleur focus
 */

import { ask } from "../model_router.js";
import { getRelevantSkills, getAllSkills, formatSkillsForPrompt } from "../skills/skillLoader.js";
...
export async function plan(intent, options = {}) {
  ...
  // Sélection pertinente : max 15 skills injectés dans le prompt
  // Évite de surcharger le contexte du LLM avec des skills hors-sujet
  ...
  try {
    ...
    // Valide contre l'ensemble complet des skills connus (pas uniquement le sous-ensemble injecté)
