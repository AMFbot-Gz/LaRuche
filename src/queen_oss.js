...
// En mode standalone, Telegram est optionnel
if (!STANDALONE) {
...
/**
 * Exécute une mission multi-agents.
 *
 * @param {string} command - La mission à exécuter
 * @param {Function} replyFn - async (text, opts?) => void — callback pour les updates
 *                             En mode Telegram : ctx.reply.bind(ctx)
 *                             En mode Standalone : noop (updates via WebSocket)
 * @param {string|null} missionId - ID de mission (mode standalone)
 * @returns {Promise<string>} Résultat final synthétisé
 */
export async function butterflyLoop(command, replyFn = async () => {}, missionId = null) {
  const start = Date.now();
  logger.info(`🦋 Mission: ${command.substring(0, 80)}`);
  broadcastHUD({ type: "mission_start", command: command.substring(0, 100), missionId });
  ...
  try {
    // 1. Stratégie
    await replyFn(`🧠 Analyse stratégique avec **${roles.strategist}**...`, {
      parse_mode: "Markdown",
    });
    broadcastHUD({ type: "thinking", agent: "Stratège", thought: "Planification...", missionId });
...
Réponds UNIQUEMENT en JSON valide.`;
    ...
    broadcastHUD({ type: "plan_ready", tasks: plan.tasks.length, missionId });
    if (missionId) appendMissionEvent(missionId, { type: "plan_ready", tasks: plan.tasks });

    // 2. Exécution parallèle
    const results = await Promise.all(
      plan.tasks.map(async (task) => {
        broadcastHUD({ type: "task_start", task: task.description.substring(0, 60), missionId });
        ...
        broadcastHUD({ type: "task_done", task: task.description.substring(0, 60), missionId });
    ...
    // 3. Synthèse
    broadcastHUD({ type: "thinking", agent: "Synthèse", thought: "Finalisation...", missionId });
...
Réponse directe, sans répéter les étapes.`;
    ...
    saveMission(missionRecord);
    broadcastHUD({ type: "mission_complete", duration, missionId });
    ...
    return `${synthesis.text}\n\n_⏱ ${(duration / 1000).toFixed(1)}s — Modèles: ${modelsUsed.join(", ")}_`;
  } catch (err) {
...
/**
 * Fonction runMission utilisée par l'API standalone
 * Exécute la mission et retourne le résultat final
 */
...
// ─── Démarrage ────────────────────────────────────────────────────────────────
logger.info("╔══════════════════════════════════════════╗");
logger.info(`║ 🐝 LaRuche OSS v3.2 — ${STANDALONE ? "Standalone    " : "Telegram mode"} ║`);
logger.info("╚══════════════════════════════════════════╝");
...
Code fonctionnel uniquement, pas d'explication.`;
    ...
    writeFileSync(
      ...
      JSON.stringify(
        { name: skillName, description: desc, version: "1.0.0", model: result.model, created: new Date().toISOString() },
        null, 2
      )
    );
